...
newNodes_really_new :: (Graph gr) => gr a b -> Positive Int -> Bool
newNodes_really_new g (Positive n) = liftA2 (&&) (all (not . (`gelem`g)))
                                                 ((n==) . length)
                                       (newNodes n g)
...
-- | Delete every possible edge, and ensure that the nodes from that
--   edge are still there.
